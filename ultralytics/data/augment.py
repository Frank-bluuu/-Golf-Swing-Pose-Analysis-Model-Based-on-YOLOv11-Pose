    ...
    def __init__(self, p=1.0):
        ...
        try:
            ...
            T = [
                ...
                # --------YOLOPose(20250530)--------
                A.HorizontalFlip(p=0.5),  # 图像翻转
                A.Affine(scale=(0.8, 1.2), translate_percent=0.1, rotate=10, shear=5, p=0.5),  # 针对细长物体的增强
                A.Perspective(scale=0.05, p=0.3),  # 轻微透视变换，模拟不同相机角度
                # 模拟不同光照条件
                ...
                A.HueSaturationValue(hue_shift_limit=20, sat_shift_limit=30, val_shift_limit=20, p=0.5),  # 色调调整
                # 增加运动模糊（模拟挥杆瞬间）
                A.MotionBlur(blur_limit=7, p=0.5),
                A.GaussianBlur(blur_limit=3, p=0.5),  # 高斯模糊（模拟运动模糊）

                # --------YOLOPose(20250601)--------
                # A.RandomResizedCrop(height=640, width=640, scale=(0.8, 1.0)),  # 随机裁剪和缩放
                A.ShiftScaleRotate(shift_limit=0.05, scale_limit=0.1, rotate_limit=15, p=0.5),  # 平移、缩放、旋转
                A.CLAHE(clip_limit=2.0, tile_grid_size=(8, 8), p=0.3),  # 对比度受限的自适应直方图均衡化
                ToTensor,
            ]

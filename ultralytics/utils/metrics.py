...
def kpt_iou_club(kpt1, kpt2, area, sigma, eps=1e-7):
    ...
    e = d_club / ((2 * sigma).pow(2) * (area[:, None, None] + eps) * 2)

    # kpt_iou_club = ((-e).exp() * kpt_club_mask[:, None]).sum(-1) / (kpt_club_mask.sum(-1)[:, None] + eps)
    return ((-e).exp() * kpt_club_mask[:, None]).sum(-1) / (kpt_club_mask.sum(-1)[:, None] + eps)
...
class PoseMetrics(SegmentMetrics):
    ...
    def process(self, tp, tp_p, tp_p_club,conf, pred_cls, target_cls, on_plot=None):
        """
